...
int main() {
    ...
    while(y < 5) scanf("%hhu",&y);
    bin(y);

    valores = alocar_vetor(y);
    esconder_bits(x,y,valores);
    printf("Valores com bits de X ocultos:");
...
unsigned char *alocar_vetor(unsigned char y) {
    ...
    for(int i = 0;i < 9;i++) *(vetor+i) = (y-4)+i;
    return vetor;
}

void esconder_bits(unsigned char x,unsigned char y,unsigned char *valores) {
    for (int i = 0;i < 9;i++) *(valores+i) = (*(valores+i)&~1)|((x >> i) & 1);
}

unsigned char bin(int num) {
    for(int i = 8;i>=0;i--) (num>>i)&1 ? putchar('1') : putchar('0');
}
